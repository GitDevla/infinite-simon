...
export default function ProfileScreen() {
	...
	const [scores, setScores] = useState<{difficulty: GameType; mode: GameMode; score: number, date: string, placement?: number }[]>([]);
	...
	const fetchScores = async () => {
		...
		return [
			...
			{difficulty: GameType.Extended, mode: GameMode.MultiPlayer, score: 15, date: "2025-10-03", placement: 2},
    ...
    const filteredScores = scores.filter(score => {
        return (modeFilter === "all" || score.mode === Number(modeFilter)) &&
					...
					<div className="mt-8 w-full">
                        ...
                        <div className="flex gap-4 mb-6 p-4 bg-bg-secondary rounded-xl">
                            <div className="flex-1">
                                <label htmlFor={modeSelectId} className="block mb-2 text-xs font-medium text-gray-400 uppercase tracking-wide">Mode</label>
                                <select
                                    id={modeSelectId}
                                    value={modeFilter}
                            ...
                            <div className="flex-1">
                                <label htmlFor={typeSelectId} className="block mb-2 text-xs font-medium text-gray-400 uppercase tracking-wide">Difficulty</label>
                                <select
                                    id={typeSelectId}
                                    value={typeFilter}
						...
						<ul className="flex flex-col gap-3 w-full">
							{filteredScores.map((score, index) => (
								<li key={index} className="flex border border-gray-600 p-4 w-full justify-between bg-bg-secondary rounded-xl bg-opacity-80 hover:scale-105 transition-transform duration-200">
									<div className="flex flex-col gap-1">
										<div className="flex items-center gap-2">
											<span className="text-xs font-medium text-gray-400 uppercase tracking-wide">Difficulty:</span>
											<span className="font-semibold text-white">{gameTypeToString(score.difficulty)}</span>
										</div>
										<div className="flex items-center gap-2">
